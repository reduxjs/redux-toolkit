 ...
 * };
 *
 * type UserAutoActions = AutoActions<UserState, "user">;
 *
 * // Results in:
 * // {
 ...
 * @public
 */
export type AutoActions<State extends any, Name extends string> = {
  [stateName in keyof State as `${typeof prefixAutoAction}${Capitalize<string & stateName>}`]: ActionCreatorWithPayload<
    State[stateName],
    Name
  >
}
...
export type AutoReducers<State> = {
  [stateName in keyof State as `${typeof prefixAutoAction}${Capitalize<string & stateName>}`]: CaseReducer<
    State,
 ...
 * 2. `AutoAction` - Action creators automatically generated from the state shape (prefixed with 'set')
 * 
 * @template CaseReducerActions - The action creators from explicit reducers
 * @template AutoAction - The auto-generated 'set' action creators from `AutoActions`
 * 
 * @example
 * ```typescript
 * const userSlice = createSlice({
 ...
 * });
 * 
 * type ManualActions = typeof userSlice.actions;
 * type AutoGeneratedActions = AutoActions<UserState, "user">;
 * 
 * // Combined actions include:
 * // - 'updateProfile' (manual)
 * // - 'setName', 'setAge', 'setActive' (auto-generated)
 * type AllActions = MergeWithAutoActions<ManualActions, AutoGeneratedActions>;
 * 
 * // Usage:
...
export type MergeWithAutoActions<
  CaseReducerActions,
  AutoAction extends AutoActions<any, string>,
> = MergeDefaults<CaseReducerActions, AutoAction>
 ...
 * @returns {AutoActions<State, Name>} An object containing reducers for each state property
 * @example
 * ```typescript
...
export const createAutoReducers = <State>(
  ...
  return stateFieldNames.reduce<Partial<UnknownRecord>>((acc, fieldName) => {
    const capitalizeFieldName = `${prefixAutoAction}${capitalize(fieldName)}`

...
type Prop = Extract<ObjectExpression['properties'][number], { value: unknown }>['value'];
...
const createAddTagTypesCall = (
  ...
  const newCall = j.callExpression(j.memberExpression(object, j.identifier('addTagTypes')), []);
  if (addTagTypes.type === 'Identifier') {
    ...
    newCall.arguments = addTagTypes.elements.filter(
      (el): el is Exclude<typeof el, RestElement | null> => !!(el && el.type !== 'RestElement')
    );
  }
  return newCall;
};
          ...
          if (endpoints.type === 'ObjectExpression') {
            for (const endpointProp of endpoints.properties) {
              if (endpointProp.type === 'ObjectProperty') {
                const endpointName =
                  endpointProp.key.type === 'Identifier' && !endpointProp.computed
                    ? j.stringLiteral(endpointProp.key.name)
                    : endpointProp.key;
                calleeObject = j.callExpression(
                  j.memberExpression(calleeObject, j.identifier('enhanceEndpoint')),
                ...
                calleeObject = j.callExpression(
                  j.memberExpression(calleeObject, j.identifier('enhanceEndpoint')),
                  [endpointName, j.arrowFunctionExpression(endpointProp.params, endpointProp.body)]
                );
              }
            }
          }

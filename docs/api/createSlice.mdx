...
`createSlice` automatically generates a reducer for each property in your slice's state.
This also gives you an automatically generated action creator for each property in your slice's state.
...
const counterSlice = createSlice({
  ...
  reducers: {
    // Custom reducer that overrides auto-generated setValue
    setValue: (state, action) => {
      // Custom logic here
      state.value = action.payload * 2
    },
    // Additional custom reducers work alongside auto-generated ones
    increment: (state) => {
      state.value += 1
    },
  },
...
// Auto-generated action creators are available alongside custom ones
const { setValue, setName, setActive, increment } = counterSlice.actions

// Usage in components
dispatch(setName('Enhanced Counter'))
...
For a state with properties:

- `name: string`
- `count: number`
- `isLoading: boolean`

The following actions are automatically generated:

- `setName(payload: string)`
- `setCount(payload: number)`
- `setIsLoading(payload: boolean)`

#### Overriding Auto-Generated Action Creators

You can override any auto-generated action creator by defining a reducer with the same name:
...
const userSlice = createSlice({
  ...
  reducers: {
    // Override auto-generated setName with custom validation
    setName: (state, action) => {
      if (action.payload.length > 0) {
        state.name = action.payload.trim()
      }
    },
    // Override auto-generated setAge with range checking
    setAge: (state, action) => {
      if (action.payload >= 0 && action.payload <= 150) {
        state.age = action.payload
      }
    },
    // Auto-generated setEmail remains as-is
  },
})
```
...
// Generated action creators:
// - setUser(payload: { id: number; name: string })
// - setSettings(payload: { theme: string; notifications: boolean })
```
...
// TypeScript knows the payload types:
// setId(payload: number)
// setName(payload: string)
// setEmail(payload: string | undefined)
```

#### Migration from Existing Code

If you're migrating existing code, the feature is opt-in and backward compatible:
...
const slice = createSlice({
  ...
  reducers: {
    ...
    // setCount and setName are auto-generated
  },
})
```
...
1. **Use for Simple Setters:** Auto-generated actions work best for simple property updates
2. **Override for Complex Logic:** Define custom reducers when you need validation, transformation, or side effects
3. **Combine Approaches:** Mix auto-generated actions with custom business logic reducers
4. **Naming Consistency:** Follow the `setPropertyName` convention for consistency

#### Limitations

- Only top-level properties generate action creators (nested properties use object-level setters)
- Property names starting with non-alphabetical characters may not work correctly
- Symbol keys are ignored

#### Generated Action Creators

Auto-generated action creators have:

- Type: `sliceName/setPropertyName`

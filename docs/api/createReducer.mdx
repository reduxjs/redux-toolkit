...
Redux [reducers](/tutorials/fundamentals/part-3-state-actions-reducers) are often implemented using a `switch` statement, with one `case` for every handled action type.

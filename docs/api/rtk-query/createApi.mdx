...
- `keepUnusedDataFor` (optional, only for query endpoints)
...
- `onQueryStarted` _(optional)_ - Available to both [queries](../../usage/rtk-query/queries.mdx) and [mutations](../../usage/rtk-query/mutations.mdx)
...
- `extraOptions` _(optional)_ - Passed as the third argument to the supplied `baseQuery` function

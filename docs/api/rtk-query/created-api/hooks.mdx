...
- **Parameters**
  - `arg`: The query argument to be used in constructing the query itself, and as a cache key for the query
    You can also pass in `skipSymbol` here as an alternative way of skipping the query, see [skipSymbol](#skipsymbol)
...
[summary](docblock://query/core/buildSelectors.ts?token=skipSymbol)
...
- **Parameters**
  ...
  - `arg`: The argument passed to the query defined in the endpoint
    You can also pass in `skipSymbol` here as an alternative way of skipping the selection, see [skipSymbol](#skipsymbol)

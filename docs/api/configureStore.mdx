...
[Redux `combineReducers` utility](/api/combinereducers).
...
Enhancers returned by this callback will be passed to [the Redux `compose` function](/api/compose), and the combined enhancer will be passed to `createStore`.

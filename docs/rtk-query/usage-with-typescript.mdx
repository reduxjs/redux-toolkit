...
## Typing Hook Results with `TypedUse*` Helpers

RTK Query exports several `TypedUse*` helper types that let you create strongly-typed versions of the auto-generated hooks. These are useful when you need to define hook wrappers, type function parameters, or create reusable utility functions that work with specific endpoints.

### Available Helper Types

The following helper types are exported from `@reduxjs/toolkit/query/react`:

#### Standard Query Hooks

| Type                              | Purpose                                       |
| --------------------------------- | --------------------------------------------- |
| `TypedUseQuery`                   | Type for the `useQuery` hook                  |
| `TypedUseQueryHookResult`         | Type for the result of `useQuery`             |
| `TypedUseQueryState`              | Type for the `useQueryState` hook             |
| `TypedUseQueryStateResult`        | Type for the result of `useQueryState`        |
| `TypedUseQueryStateOptions`       | Type for options passed to `useQueryState`    |
| `TypedUseQuerySubscription`       | Type for the `useQuerySubscription` hook      |
| `TypedUseQuerySubscriptionResult` | Type for the result of `useQuerySubscription` |
| `TypedQueryStateSelector`         | Type for `selectFromResult` callbacks         |

#### Lazy Query Hooks

| Type                            | Purpose                                         |
| ------------------------------- | ----------------------------------------------- |
| `TypedUseLazyQuery`             | Type for the `useLazyQuery` hook                |
| `TypedUseLazyQueryStateResult`  | Type for the result state of `useLazyQuery`     |
| `TypedUseLazyQuerySubscription` | Type for the `useLazyQuerySubscription` hook    |
| `TypedLazyQueryTrigger`         | Type for the trigger function from lazy queries |

#### Mutation Hooks

| Type                     | Purpose                                      |
| ------------------------ | -------------------------------------------- |
| `TypedUseMutation`       | Type for the `useMutation` hook              |
| `TypedUseMutationResult` | Type for the result of `useMutation`         |
| `TypedMutationTrigger`   | Type for the trigger function from mutations |

#### Infinite Query Hooks

| Type                                      | Purpose                                              |
| ----------------------------------------- | ---------------------------------------------------- |
| `TypedUseInfiniteQuery`                   | Type for the `useInfiniteQuery` hook                 |
| `TypedUseInfiniteQueryHookResult`         | Type for the result of `useInfiniteQuery`            |
| `TypedUseInfiniteQueryState`              | Type for the `useInfiniteQueryState` hook            |
| `TypedUseInfiniteQueryStateResult`        | Type for the result of `useInfiniteQueryState`       |
| `TypedUseInfiniteQueryStateOptions`       | Type for options passed to infinite query state      |
| `TypedUseInfiniteQuerySubscription`       | Type for the `useInfiniteQuerySubscription` hook     |
| `TypedUseInfiniteQuerySubscriptionResult` | Type for the result of infinite query subscription   |
| `TypedInfiniteQueryStateSelector`         | Type for infinite query `selectFromResult` callbacks |
| `TypedLazyInfiniteQueryTrigger`           | Type for lazy infinite query triggers                |

Each helper type takes three required generic parameters:

- `ResultType`: The type of data returned by the endpoint
- `QueryArg`: The type of argument passed to the query/mutation
- `BaseQuery`: The type of your base query function (typically `typeof fetchBaseQuery` or your custom base query)

### Basic Usage

```ts title="Using TypedUseQueryHookResult to type a function parameter"
...
type PostsBaseQuery = ReturnType<typeof fetchBaseQuery>
...
// Use the helper to type a function that accepts hook results
function processPostsResult(
  result: TypedUseQueryHookResult<Post[], void, PostsBaseQuery>,
) {
  if (result.isSuccess) {
    console.log('Posts loaded:', result.data.length)
  }
}
```

### Creating Pre-Typed Hook Wrappers

A common use case is creating wrapper functions around hooks with default options. The `TypedUseQueryStateOptions` helper is useful for typing the options parameter:

```ts title="Creating a useQuery wrapper with default options"
import type {
  SubscriptionOptions,
...
// Create a wrapper hook with default options
...
### Typing `selectFromResult` Functions

When defining `selectFromResult` callbacks separately from the hook call, you can use `TypedUseQueryStateResult` or `TypedUseMutationResult` to type the input parameter:

```ts title="Typing a selectFromResult function"
...
type PostsApiBaseQuery = ReturnType<typeof fetchBaseQuery>
...
// Define selectFromResult separately with proper typing
const selectPostById = (id: number) => {
  return (result: TypedUseQueryStateResult<Post[], void, PostsApiBaseQuery>) => ({
    post: result.data?.find((post) => post.id === id),
  })
}

function PostById({ id }: { id: number }) {
  const { post } = useGetPostsQuery(undefined, {
    selectFromResult: selectPostById(id),
  })
...
Use these helper types when you need to:

- **Type function parameters**: When writing utility functions that accept hook results as arguments
- **Create hook wrappers**: When building wrapper hooks with default options or additional logic
- **Define `selectFromResult` callbacks separately**: When extracting selector functions for reuse
- **Type external state**: When storing hook results in external state management

For most typical usage of RTK Query hooks, you won't need these types - the auto-generated hooks are already fully typed. These helpers are primarily useful for advanced patterns involving hook composition or when building abstractions on top of RTK Query.

### Using the `Typed` Namespace (Recommended)

RTK Query also exports a `Typed` namespace that provides the same helper types with better discoverability. Instead of importing individual `TypedUse*` types, you can import the `Typed` namespace and access all related types through nested namespaces:

```ts title="Using the Typed namespace"
...
type MyBaseQuery = ReturnType<typeof fetchBaseQuery>

// Instead of importing TypedUseQueryHookResult directly:
type PostsResult = Typed.UseQuery.Result<Post[], void, MyBaseQuery>

// Type a selectFromResult callback:
type PostsStateResult = Typed.UseQuery.StateResult<Post[], void, MyBaseQuery>

// Type a lazy query trigger:
type PostsTrigger = Typed.UseLazyQuery.Trigger<Post[], void, MyBaseQuery>

// Type a mutation result:
type AddPostResult = Typed.UseMutation.Result<Post, string, MyBaseQuery>
```

The namespace approach makes it easier to discover available types through IDE autocomplete - just type `Typed.` and your editor will show all available hook namespaces (`UseQuery`, `UseLazyQuery`, `UseMutation`, etc.), then type `.` again to see available types within that namespace.

#### Namespace Reference

| Namespace                            | Available Types                                                                         |
| ------------------------------------ | --------------------------------------------------------------------------------------- |
| `Typed.UseQuery`                     | `Hook`, `Result`, `StateResult`, `SubscriptionResult`, `Options`, `Selector`            |
| `Typed.UseQueryState`                | `Hook`, `Result`, `Options`                                                             |
| `Typed.UseQuerySubscription`         | `Hook`, `Result`                                                                        |
| `Typed.UseLazyQuery`                 | `Hook`, `Result`, `Trigger`                                                             |
| `Typed.UseLazyQuerySubscription`     | `Hook`                                                                                  |
| `Typed.UseInfiniteQuery`             | `Hook`, `Result`, `StateResult`, `SubscriptionResult`, `Options`, `Selector`, `Trigger` |
| `Typed.UseInfiniteQueryState`        | `Hook`, `Result`                                                                        |
| `Typed.UseInfiniteQuerySubscription` | `Hook`, `Result`                                                                        |
| `Typed.UseMutation`                  | `Hook`, `Result`, `Trigger`                                                             |

Both the flat `TypedUse*` exports and the `Typed.*` namespace are available and will continue to be supported. The namespace approach is recommended for better discoverability.

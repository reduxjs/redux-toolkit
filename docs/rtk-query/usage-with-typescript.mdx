...
directly. If an unexpected error is thrown by user code rather than a graceful handled error,
that error will be transformed into a `SerializedError` shape. Users should ensure that they are checking which
kind of error they are dealing with prior to attempting to access it's properties. This can be done in a type safe manner either

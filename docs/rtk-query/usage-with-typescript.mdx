...
In the same manner as the core Redux Toolkit package, RTK Query is also written in TypeScript, with it's API designed for seamless use in TypeScript applications.
...
As mentioned in [Typing query and mutation endpoints](#typing-query-and-mutation-endpoints), a `queryFn` will receive it's result & arg types from the generics provided to the corresponding built endpoint.
...
An error case for the example above using `queryFn` and the error type from `fetchBaseQuery` could look like so:

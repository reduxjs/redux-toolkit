...
Passing a `number` as a value in seconds will use the following behavior: At the time a query subscription is created, if there is an existing query in the cache, it will compare the current time vs the last fulfilled timestamp for that query, and refetch if the provided amount of time in seconds has elapsed. If there is no query, it will fetch the data. If there is an existing query, but the amount of time specified since the last query has not elapsed, it will serve the existing cached data.
...
<iframe
  src="https://codesandbox.io/embed/rtk-query-cache-subscription-lifetime-example-77tn4?fontsize=12&hidenavigation=1&theme=dark&module=%2Fsrc%2Fservice%2Findex.ts"
  style={{
    width: '100%',
    height: '800px',
    border: 0,

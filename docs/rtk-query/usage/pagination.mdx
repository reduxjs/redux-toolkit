...
The [`infinite-queries` example project](https://github.com/reduxjs/redux-toolkit/tree/master/examples/query/react/infinite-queries) includes a `pagination` feature that shows this pattern. On the initial query you'll see `Loading`; as you move forward, the next/previous buttons act as a _fetching_ indicator while any non-cached page is fetched. When you go back, the cached data is served immediately.

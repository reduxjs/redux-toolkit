...
To summarize the above behavior - the returned values must be correctly memoized. See also [Deriving Data with Selectors](/usage/deriving-data-selectors) and [Redux Essentials - RTK Query Advanced Patterns](/tutorials/essentials/part-8-rtk-query-advanced#selecting-values-from-results) for additional information.
...
This example demonstrates request deduplication and caching behavior. Several `Pokemon` components each call the same query hook:

```tsx no-transpile title="src/Pokemon.tsx"
...
export default function App() {
  ...
  useEffect(() => {
    setTimeout(() => setPokemon((prev) => [...prev, 'bulbasaur']), 1500)
    setTimeout(() => setPokemon((prev) => [...prev, 'pikachu']), 2000)
  }, [])
...
If you run this and watch the Network panel:
...
2. A second later, another `Pokemon` component is rendered with 'bulbasaur'
   - This one never shows 'Loading...' and no new network request happens. It's using the cache.
3. A moment after that, a `Pokemon` component for 'pikachu' is added, and a new request happens.
4. Clicking 'Add bulbasaur' adds more components without any new requests.
5. Clicking 'Refetch' on any `Pokemon` updates all instances of that `Pokemon` with one request.

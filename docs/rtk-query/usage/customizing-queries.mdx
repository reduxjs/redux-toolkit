...
While there is less need to store the response in a [normalized lookup table](/usage/structuring-reducers/normalizing-state-shape) with RTK Query managing caching data, `transformResponse` can be leveraged to do so if desired.

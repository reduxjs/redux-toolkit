...
Note that tags provided in an simple array (such as `['Post', 'User']`) and not a callback are only considered for successful requests. In order to provide tags for failed requests the callback has to be used. With the callback it is also possible to provide different tags for failed or successful requests.

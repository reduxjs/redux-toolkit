...
Instead of dispatching an action from `onQueryStarted`, the `authSlice` can listen for the login endpoint's `matchFulfilled` matcher in `extraReducers`. The rest of the app is the same as the previous example.

```ts no-transpile title="src/features/auth/authSlice.ts"
...
RTK Query's core APIs work without React. This example uses `api.endpoints.getPokemon.initiate()` and manual subscription handling from a Svelte component:

[RTK Query Svelte Example on CodeSandbox](https://codesandbox.io/s/svelte-app-rtk-simplequery-demo-f1ueb)

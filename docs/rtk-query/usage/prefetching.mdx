...
Picking up on our last example, we automatically `prefetch` the next page whenever the current page loads, giving the appearance of no network delay.
...
const PostList = () => {
  ...
  // ...render the list and the prev/next buttons
}
```

### Prefetching All Known Pages

After the first query initialized by `useQuery` runs, we can fetch all remaining pages at once. `force: true` makes sure each page is fetched even if it is already in the cache.
...
const PostList = () => {
  ...
  useEffect(() => {
    if (!hasPrefetchedAll && posts && posts.total_pages > 1) {
      for (let i = 1; i <= posts.total_pages; i++) {
        prefetchPage(i, { force: true })
      }
      setHasPrefetchedAll(true)
    }
  }, [posts, prefetchPage, hasPrefetchedAll])
